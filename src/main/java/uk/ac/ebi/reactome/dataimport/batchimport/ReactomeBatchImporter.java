package uk.ac.ebi.reactome.dataimport.batchimport;
...
/**
 * This component is used to batch import Reactome dataimport into neo4j.
 * This dataimport utilizes the Neo4j BatchInserter and the Reactome MySql adapter.
 * WARNING: The BatchInserter is not thread save, not transactional, and can not enforce any constraints
 *          while inserting dataimport.
 * WARNING: DATA_DIR folder will be deleted at the start of dataimport import
 *
 * Created by:
...
//
                importGkInstance(instance,true);
    ...
    private Long importGkInstance(GKInstance instance,Boolean tlp) throws ClassNotFoundException {
        ...
        setUpMethods(clazz);
        Long id = saveDatabaseObject(instance, clazz, tlp);
...
//                        }
     ...
     * @return Neo4j native id (generated by the BatchInserter)
     */
    private Long saveDatabaseObject(GKInstance instance, Class clazz, Boolean tlp) throws IllegalArgumentException{
    ...
    private void saveRelationships(Long oldId, Collection objects, String relationName) throws ClassNotFoundException {
        ...
        for (Long dbId : stoichiometryMap.keySet()) {
            ...
            if (!dbIds.containsKey(dbId)) {
                newId = importGkInstance(instance,false);
                instance.deflate();
            } else {
