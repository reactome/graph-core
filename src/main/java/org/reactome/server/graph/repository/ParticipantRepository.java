...
public class ParticipantRepository {
    ...
    public Collection<Participant> getParticipants(Long dbId) {
        String query = " " +
                ...
                "OPTIONAL MATCH (m)-[:hasMember|hasComponent|hasCandidate|RNAMarker|proteinMarker|repeatedUnit*]->(pe:PhysicalEntity)-[:referenceEntity]->(re2:ReferenceEntity) " +
                "WITH DISTINCT m,  " +
    ...
    public Collection<Participant> getParticipants(String stId) {
        String query = " " +
                "MATCH (n:DatabaseObject{stId:$stId})-[:hasEvent|input|output|catalystActivity|physicalEntity|entityFunctionalStatus|diseaseEntity|regulatedBy|RNAMarker|proteinMarker|regulator*]->(m:PhysicalEntity) " +
                "OPTIONAL MATCH (m)-[:referenceEntity]->(re1:ReferenceEntity) " +
                "OPTIONAL MATCH (m)-[:hasMember|hasComponent|hasCandidate|RNAMarker|proteinMarker|repeatedUnit*]->(pe:PhysicalEntity)-[:referenceEntity]->(re2:ReferenceEntity) " +
                "WITH DISTINCT m, " +
                "              CASE WHEN re1 IS NULL THEN re2 ELSE re1 END AS re, " +
